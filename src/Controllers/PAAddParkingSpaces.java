...
public class PAAddParkingSpaces extends JFrame {
			...
			public void run() {
				try {
					if (ParkingAuthorityController.getUser() != null) {
						PAAddParkingSpaces frame = new PAAddParkingSpaces();
	...
	private void initialize() {
		...
		contentPane.setBorder(new EmptyBorder(100, 100, 701, 527));
		...
		btnAddBooking.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(!parkingSpace.equals("")) {
					if(user.addSpace(Integer.parseInt(parkingSpace))) {
						JOptionPane.showMessageDialog(null, "Space Added", "Success",JOptionPane.PLAIN_MESSAGE);
